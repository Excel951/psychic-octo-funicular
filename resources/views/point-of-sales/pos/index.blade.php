<x-pos-layout>
    ...
    <div class="flex-grow flex">
            ...
            <div class="h-full overflow-hidden mt-4">
                <div class="h-full overflow-y-auto px-2">
                    ...
                    <div class="grid grid-cols-4 gap-4 pb-3">
                        ...
                        <div class="select-none cursor-pointer transition-shadow overflow-hidden rounded-2xl bg-white shadow hover:shadow-lg product-item"
            ...
            <div x-show="isShowModalReceipt" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10"
                ...
                <div id="receipt-content" class="text-left w-full text-sm p-6 overflow-auto">
                    <div class="text-center">
                        <img src="{{ asset('img/receipt-logo.png') }}" alt="Tailwind POS"
        ...
        <div x-show="isShowModalQr"
            class="fixed w-full h-screen left-0 top-0 z-10 flex flex-wrap justify-center content-center p-24">
            <div x-show="isShowModalQr" class="fixed glass w-full h-screen left-0 top-0 z-0"
                x-on:click="closeModalQr()" x-transition:enter="transition ease-out duration-100"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0"></div>
            ...
            <div x-show="isShowModalEndSession" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10"
                ...
                <div class="p-4 w-full">
                        <div class="mb-4">
                            ...
                            <input type="text" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value="Rp. {{ number_format($session->initial_cash, 0, ',', '.') }}" disabled>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700" for="payment-method">
                                Cash In
                            </label>
                            <input type="text" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value="Rp. {{ number_format($session->cash_in, 0, ',', '.') }}" disabled>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700" for="payment-method">
                                Ending Cash System
                            ...
                            @endphp
                            <input type="text" name="ending_cash_system" id="ending_cash_system" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value="Rp. {{ number_format($endingCashSytem, 0, ',', '.') }}" disabled>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700" for="payment-method">
                                Ending Cash Actual
                            </label>
                            <input type="text" name="ending_cash_actual" id="ending_cash_actual" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" placeholder="Masukkan kas aktual">
                        </div>
                        <button class="bg-cyan-500 text-white text-lg px-4 py-3 rounded-2xl w-full focus:outline-none"
